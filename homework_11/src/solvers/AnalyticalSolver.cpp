...
{
  if (!solveCommonBallistics(context.ammo, context.droneCfg, context.telemetry.z)) {
    throw std::logic_error(std::format("Connot solve balistics for given ammo {}", context.ammo.name));
  }

  return solveTargetFireParams(context);
}

bool AnalyticalSolver::setTimeOfFlight(const dlink::AmmoCfg& ammo, double altitude, double speed)
{
...
bool AnalyticalSolver::setHorizontalFlightDistance(const dlink::AmmoCfg& ammo, double speed)
{
  constexpr double g{9.81f};
  const double V0 = speed;
  const double sq_m = std::pow(ammo.mass, 2);
...
bool AnalyticalSolver::solveCommonBallistics(const dlink::AmmoCfg& ammo, const dlink::DroneCfg& drone, float altitude)
{
  if (setTimeOfFlight(ammo, altitude, drone.attackSpeed) && setHorizontalFlightDistance(ammo, drone.attackSpeed)) {
    return true;
  }
  else {
    return false;
